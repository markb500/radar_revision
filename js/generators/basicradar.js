// js/generators/basicradar.js
// Clean ES module for Basic Radar questions
import { rndgen, dp, thouSep, QLimitRepeats, isCanvasBlank, images } from '../utils.js';

const NOTES = 'images/20200323-RadarBk1BasicRadCIv1_5-APO.pdf';
const CANVAS_W = 500;
const CANVAS_H = 300;

/** @type {number[]} recent question ids to limit repeats */
let recentIds = [];

/**
 * Build one Basic Radar question.
...
export function generate() {
  ...
  let solution = '';
  let notesLink = NOTES;
  let txrxtime, slantrange, pp, pw, prf, mp, ps;

  // Offscreen canvases for optional diagrams
  ...
  // Local aliases matching original code style inside the switch
  let sumq = '';
  let suma = '';
  ...
  switch (sum) {
        ...
        case 1:
            ...
            suma += "&=\\underline{\\mathbf{" + (slantrange / 1000) + "\\ Km}}\\\\[5pt]";
            suma += "\\mathbf{OR}\\\\[5pt]";
            suma += "slant\\ range\\ (km) &=\\frac{Tx/Rx\\ time\\ (&#956s)}{6.7}\\\\[5pt]";
            suma += "&=\\frac{" + txrxtime + "}{6.7}\\\\[5pt]";
            suma += "&=\\underline{\\mathbf{" + dp(txrxtime / 6.7, 1, -1) + "\\ Km}}";
            suma += "\\end{aligned}$$(Difference in solutions due to rounding to 6.7 &#956s)";
            break;
        case 2:
            notesLink = "images/20200323-RadarBk1BasicRadCIv1_5-APO.pdf#page=7";
            ...
            suma += "&=\\underline{\\mathbf{" + txrxtime + "\\ &#956s}}\\\\[5pt]";
            suma += "\\mathbf{OR}\\\\[5pt]";
            suma += "Distance\\ travelled\\ by\\ pulse &= 2\\times slant\\ range\\\\[5pt]";
            suma += "&=2\\times" + slantrange + "\\times 10^3\\\\[5pt]";
            suma += "&=" + (2 * slantrange) + "\\ Km\\\\[5pt]";
            suma += "\\\\[5pt]";
            suma += "slant\\ range &= speed\\times time\\\\[5pt]";
            suma += "\\frac{slant\\ range}{speed} &= time\\\\[5pt]";
            suma += "\\frac{" + (slantrange * 2) + "\\times 10^3}{300\\times 10^{6}} &=" + dp((slantrange * 2 * 1000) / 300, 1, -1) + "\\ &#956s";
            suma += "\\end{aligned}$$(Difference in solutions due to rounding to 6.7 &#956s)";
            break;
        case 3:
        ...
        case 13:
            notesLink = "images/20200323-RadarBk1BasicRadCIv1_5-APO.pdf#page=16";
            sumq += "With reference to the Basic Pulsed Radar Block Diagram shown, identify the blocks labelled A - G.";
        ...
        case 15:
            ...
            ctx.drawImage(images.jetsclosing, 0, 0, 500, 100);
        ...
        case 16:
            ...
            ctx.drawImage(images.jetsopening, 0, 0, 500, 100);
            ctx.font = "20px Comic Sans MS";
            ctx.textAlign = "center";
